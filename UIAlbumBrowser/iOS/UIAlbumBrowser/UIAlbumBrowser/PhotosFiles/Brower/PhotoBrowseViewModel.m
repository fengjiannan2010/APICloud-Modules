...
@implementation PhotoBrowseViewModel
...
-(void)highQualityStatusShouldChanged:(UIScrollView *)scrollView
{
    ...
    //变换标志位
    [PhotoCacheManager sharedInstace].isHightQuarity = !isHightQuarity;
    ...
    printf("change to %s!\n",[PhotoCacheManager sharedInstace].isHightQuarity ? "Yes" : "No");
}
...
{
    self.ritl_browerQuarityChangedBlock([PhotoCacheManager sharedInstace].isHightQuarity);
}
...
- (void)ritl_checkHightQuaratyStatusChanged:(NSUInteger)index
{
    ...
    //如果是高清状态
    if ([PhotoCacheManager sharedInstace].isHightQuarity)
    {
        //获得当前的资源对象
...
{
    //获得当前资源的状态
    return [PhotoCacheManager sharedInstace].assetIsSelectedSignal[index];
}
...
- (UIImage *)ritl_imageForCurrentAsset:(BOOL)isSelected
{
    ...
    return isSelected ?  [UIImage imageWithContentsOfFile:selectedImgPath] : [UIImage imageWithContentsOfFile:deselectedImgPath];
}
