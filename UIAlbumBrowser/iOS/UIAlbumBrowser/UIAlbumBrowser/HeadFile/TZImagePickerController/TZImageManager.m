...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    if ([presets containsObject:presetName]) {
        ...
        NSString *outputPath = [NSHomeDirectory() stringByAppendingFormat:@"/Library/Caches/%@.mp4", [formater stringFromDate:[NSDate date]]];
        // NSLog(@"video outputPath = %@",outputPath);
//        NSString *outputPath = [NSString stringWithFormat:@"%@/Library/Caches/movie_%@.mp4",NSHomeDirectory(), [formater stringFromDate:[NSDate date] withFormat:@"yyyy-MM-dd"]];
